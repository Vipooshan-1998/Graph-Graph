...
from torch_geometric.nn import GCNConv, GATv2Conv, SAGPooling, InstanceNorm, TopKPooling, global_max_pool

class SpaceTempGoG_detr_dota(nn.Module):
    def __init__(self, input_dim=2048, embedding_dim=128, img_feat_dim=2048, num_classes=2, dropout=0.3):
        super(ImprovedAccidentPrevention, self).__init__()
        self.input_dim = input_dim
        self.num_heads = 2  # Increased heads for better attention
        ...
        # GNN for object-level graph
        self.gc1_spatial = GCNConv(embedding_dim * 2 + embedding_dim // 2, embedding_dim // 2)
        self.gc1_norm1 = InstanceNorm(embedding_dim // 2)
        self.gc1_temporal = GCNConv(embedding_dim * 2 + embedding_dim // 2, embedding_dim // 2)
        self.gc1_norm2 = InstanceNorm(embedding_dim // 2)

        # Improved pooling (TopKPooling + SAGPooling)
        self.pool_topk = TopKPooling(embedding_dim, ratio=0.8)
        self.pool_sag = SAGPooling(embedding_dim, ratio=0.7)

        # Image feature processing with normalization
        self.img_fc = nn.Linear(img_feat_dim, embedding_dim * 2)
        self.img_bn = nn.BatchNorm1d(embedding_dim * 2)

        # GATv2 for better graph attention
        self.gc2_sg = GATv2Conv(embedding_dim, embedding_dim // 2, heads=self.num_heads)
        self.gc2_norm1 = InstanceNorm((embedding_dim // 2) * self.num_heads)
        self.gc2_i3d = GATv2Conv(embedding_dim * 2, embedding_dim // 2, heads=self.num_heads)
        self.gc2_norm2 = InstanceNorm((embedding_dim // 2) * self.num_heads)
        ...
        self.attn = nn.MultiheadAttention(embed_dim=embedding_dim, num_heads=2, dropout=dropout)
    ...
    def forward(self, x, edge_index, img_feat, video_adj_list, edge_embeddings, temporal_adj_list, temporal_edge_w, batch_vec):
        ...
        # Spatial and Temporal Graph Embeddings with Attention
        n_embed_spatial = self.relu(self.gc1_norm1(self.gc1_spatial(x, edge_index, edge_weight=edge_embeddings[:, -1])))
        n_embed_temporal = self.relu(self.gc1_norm2(self.gc1_temporal(x, temporal_adj_list, temporal_edge_w)))
        n_embed = torch.cat((n_embed_spatial, n_embed_temporal), 1)

        # Apply Improved Pooling Strategy
        n_embed, edge_index, _, batch_vec, _, _ = self.pool_topk(n_embed, edge_index, None, batch_vec)
        n_embed, edge_index, _, batch_vec, _, _ = self.pool_sag(n_embed, edge_index, None, batch_vec)
        g_embed = global_max_pool(n_embed, batch_vec)

        # Process I3D feature
        img_feat = self.relu(self.img_bn(self.img_fc(img_feat)))
        img_feat = self.dropout_layer(img_feat)

        # Attention Fusion for Graph and Image Features
        g_embed = g_embed.unsqueeze(0)  # Convert to (1, batch, dim)
        img_feat = img_feat.unsqueeze(0)
        attn_output, _ = self.attn(g_embed, img_feat, img_feat)
        attn_output = attn_output.squeeze(0)  # Convert back

        # Frame Embedding
        frame_embed_sg = self.relu(self.gc2_norm1(self.gc2_sg(attn_output, video_adj_list)))
        frame_embed_img = self.relu(self.gc2_norm2(self.gc2_i3d(img_feat, video_adj_list)))
        frame_embed_ = torch.cat((frame_embed_sg, frame_embed_img), 1)
        frame_embed_ = self.dropout_layer(frame_embed_)
