...
def forward(self, x, edge_index, img_feat, video_adj_list, edge_embeddings,
    ...
    # -----------------------
    # Parallel Decoder branch
    # -----------------------
    decoder_input = self.decoder_fc(img_feat).unsqueeze(0)
    decoder_out = self.img_feat_decoder(tgt=decoder_input, memory=decoder_input)

    # Expand decoder output to match frame/node count
    num_frames = frame_embed_sg.size(0)
    decoder_out_expanded = decoder_out.repeat(num_frames, 1)  # (num_frames, embedding_dim*2)
    ...
    # Concatenate all features
    frame_embed_ = torch.cat((frame_embed_sg, frame_embed_img, img_feat_fusion, decoder_out_expanded), 1)
